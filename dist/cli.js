...
const StrangerStrings_1 = require("./StrangerStrings");
...
async function analyzeCommand(input, options) {
    const analyzer = new StrangerStrings_1.StrangerStrings();
    // Load model
...
async function infoCommand(options) {
    ...
    for (let i = 4; i <= 20; i++) {
        const threshold = i >= constants_1.NG_THRESHOLDS.length ? constants_1.MAX_NG_THRESHOLD : constants_1.NG_THRESHOLDS[i];
        console.log(`  Length ${i.toString().padStart(2)}: ${threshold.toFixed(3)}`);
    }
    console.log(`  Length 50+: ${constants_1.NG_THRESHOLDS[50] || constants_1.MAX_NG_THRESHOLD}`);
    console.log(`  Length 100+: ${constants_1.MAX_NG_THRESHOLD}`);
}
